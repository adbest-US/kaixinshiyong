...
function ajax_enter(content,url) {
	art.dialog.confirm(content,function(){
	...
	return {
		...
		buy: function(oid) {
			...
			if (_order.pro.mod == 'rebate') {
				...
				order.trial_url(oid);
			}else if(_order.pro.mod == 'commission') {
				order.commission_url(oid);
		...
		rebate_url : function(oid) {
			...
			switch(_order.type) {
				...
				case 'search': //搜索下单
					...
					if (_order.pro.goods_rule.keyword2) {
			...
			art.dialog({
				...
				ok : function() {
					...
					var diy_form='<form><table>'
								 ...
								 +					'<p>2、若单号被审核有误，请及时进行申诉或修改，逾期将无法领回划算金（建议平时经常登录网站查看站内信提醒哦！）<a style="color:#09f;display:block;" href="'+site.site_root+'/help/show/?catid=6&id=43" target="_blank">如何获取订单编号？</a></p>'
								 +				'</td>'
								 + 			'</tr>'
								 + 		'</tbody>'
								 + '</table></form>';
		...
		commission_url : function(oid) {
			...
			buy_succes += '<font class="font f_bg_yes font_2">抢购成功，请使用搜索方式下单</font>';
			buy_succes += '<ul class="hint_text_2 margin_b_10"><p>搜索下单提示：</p>';
			buy_succes += '<li>1、进入<em><a href="'+_order.shop_source.url+'" target="_blank">'+_order.shop_source.name+'网首页</a></em>，搜索关键词：<b style="color:red;">'+_order.pro.keyword+'</b>。</li>';
			buy_succes += '<li>2、请使用<em>Ctrl+F 查找</em>旺旺名称:<span style="color:red;">'+_order.pro.goods_wangwang+'</span>;进入店铺找到活动宝贝，然后进行下单。</li>';
			var sort = ''
			if (_order.pro.sort ==  1 ) {
				sort = '综合';
			}
			if (_order.pro.sort ==  2 ) {
				sort = '人气';
			}
			if (_order.pro.sort ==  3 ) {
				sort = '销量';
			}
			if (_order.pro.sort ==  4 ) {
				sort = '信用';
			}
			if (_order.pro.sort ==  5 ) {
				sort = '最新';
			}
			if (_order.pro.sort ==  6 ) {
				sort = '价格';
			}
			...
			art.dialog({
				...
				ok : function() {
					...
					var diy_form='<form><table>'
								 ...
								 +				'<td style="color:#999;">'
								 +					'<p>1、请填写已付款的订单编号，若填入未付款单号，属于违规行为且将无法获得返款；<a style="color:#09f;" href="'+site.site_root+'/help/show/?catid=59&id=143" target="_blank">填写的订单号规则？</a></p>'
								 +					'<p>2、若单号被审核有误，请及时进行申诉或修改，逾期将无法领回闪电佣金（建议平时经常登录网站查看站内信提醒哦！）<a style="color:#09f;display:block;" href="'+site.site_root+'/help/show/?catid=6&id=43" target="_blank">如何获取订单编号？</a></p>'
								 +				'</td>'
								 + 			'</tr>'
								 + 		'</tbody>'
								 + '</table></form>';
		...
		trial_url : function(oid) {
			...
			/*红包金额*/
			goods_bonus = _order.goods_bonus > 0 ? _order.goods_bonus: 0;

			/*返还金额*/
			fan = Number(_order.goods_price) + Number(goods_bonus);
			...
			switch(_order.type) {
				...
				case 'search': //搜索下单
					...
					buy_succes += '<ul class="hint_text_2 margin_b_10"><p>搜索下单流程：</p>';
					buy_succes += '<li>1、进入<em><a href="'+_order.shop_source.url+'" target="_blank">'+_order.shop_source.name+'网首页</a></em>，搜索关键词：<b style="color:red;">'+_order.pro.goods_rule.keyword+'</b>。</li>';
				    typeof(_order.pro.goods_rule.sort) == "undefined" ? "" :	buy_succes += '<li>2、按照 <span style="color:red;">'+ _order.pro.goods_rule.sort +' </span>排序,</li>';
                    typeof(_order.pro.goods_rule.address) == "undefined" ? "" : buy_succes += '<li>3、商品位置大约在<span style="color:red;">'+ _order.pro.goods_rule.address +'</span>,</li>';
                    typeof(_order.pro.goods_ww) == "undefined" ? "" : buy_succes += '<li>4、请认准掌柜旺旺 <span style="color:red;">'+ _order.pro.goods_ww +' </span>,</li>';

                    console.log(_order.pro)
                    typeof(_order.pro.goods_rule.collect) == "undefined" || _order.pro.goods_rule.collect == 0  ? "" : buy_succes += '<li>5、进入店铺之后<span style="color:red;">先收藏本活动商品</span></li>';

                    buy_succes += '<br/><li>搜索提示：</li>';
                    buy_succes += '<li>1.<span style="color:red;">'+ _order.pro.goods_rule.keyword2 +'</span></li>';
                    buy_succes += '<li>2.可使用<span style="color:red;">Ctrl + F</span>快速查找关键词 和 掌柜旺旺</li>';
		...
		fill_trade_no : function(oid, cause, in_time, order_fill_interval, get_order_number_help) {
			...
			var diy_form = '<form><table>'
						 ...
						 +				'<tr id="js_images" style="display:none;">'
						 + 				'<td>订单截图：</td>'
						 + 				'<td id="imgurl" style="padding-top:10px;font-weight:bold;color:#000;">'
						 +		        '<img id="images" src="/static/js/tool/signIn_14.jpg" width="100" height="50" /><input type="hidden" id="urls" value="" />'
						 +			'</td>'
						 + 			'</tr>'
						 ...
						 +				'<td style="color:#999;">'
						 +					'<p>1、请填写已付款的订单编号，若填入未付款单号，属于违规行为且将无法获得返款；<a style="color:#09f;" href="'+site.site_root+'/help/show/?catid=59&id=143 " target="_blank">填写的订单号规则？</a></p>'
						 +					'<p>2、若单号被审核有误，请在 '+act_config.buyer_check_update_order_sn+' 小时内进行申诉或修改，逾期将无法领回划算金（建议平时经常登录网站查看站内信提醒哦！）<a style="color:#09f;display:block;" href="http://help.zhonghuasuan.com/buyer/category/63/81/124" target="_blank">如何获取订单编号？</a></p>'
						 +				'</td>'
						 + 			'</tr>'
						 + 		'</tbody>'
						 + '</table></form>';
			...
			art.dialog({
				...
				init	: function(){
					var dialog = this;
					if (order.mod == 'commission') {
						$('#js_images').show();
					}

					uploader("#imgurl","#urls","#images");
		...
		add_show : function(oid) {
			...
			html.push('<tr><th>照片:</th><td id="imgurl" ><div class="sub_btn" name="uploadify" id="img1"><img id="imgsrc" src="/static/js/tool/signIn_14.jpg" width="100" height="50" /><input type="hidden" id="file_url" value="" /></div></td></tr>');
			...
			art.dialog({
				...
				init : function() {
					var win = this;
					var form = this.DOM.content.find('form');
					uploader("#imgurl","#file_url","#imgsrc");
					form.submit(function() {
						var imgs = $('#file_url').val();
		...
		/* 完成记录 */
		seller_complete_log : function(oid) {
			var order = get_order(oid);
			$.post('?m=Member&c=Order&a=seller_complete_log', {
				oid:oid,
				userid:order.userid,
				...
				modelid:order.modelid
					...
					if (ret.status == 1) {
						for (var i = 0, l = ret.info.length; i < l; i++) {
							html.push('<tr><td>' + ret.info[i].title + '</td><td>' + ret.info[i].goods_price + '</td><td>' + ret.info[i].complete_time + '</td></tr>');
						}
						html = '<div class="operate-log"><table class="ui-table"><tr><th>商品名称</th><th>下单价</th><th>完成时间</th></tr>' + html.join('') + '</table></div>';
						art.dialog({
							lock : true,
							fixed : true,
							title : '操作记录',
		...
		/* 黑名单 */
		/*blacklist : function(oid) {
			var order = get_order(oid);
			$.post('?m=Member&c=Order&a=blacklist', {
				order_id:oid,
			},function(ret) {
					var html = [];
					if (ret.status == 1) {
		...
		blacklist : function(oid) {
			...
			var html = [];
			html.push('<form class="woyao-show" method="post" enctype="multipart/form-data"><table>');
			html.push('<tr><th>拉黑原因:</td><td><br/><textarea name="content" rows="6" cols="45"></textarea></td></tr>');
			html.push('</table></form>');
			art.dialog({
				lock : true,
				fixed : true,
				...
				init : function() {
					...
					form.submit(function() {
						...
						$.ajax({
							type : "POST",
							url  : site.site_root+"index.php?m=member&c=order&a=blacklist",
							data : {order_id:oid,cause:text},
							dataType : 'JSON',
							error:function(ret){
								set_btn_state(win, true);
								error_no('系统错误，请稍后重试。');
		...
		user_edit_trade_no :function(oid){
			...
			var diy_form = '<form><table>'
						 ...
						 +					'<p>2、若单号被审核有误，请及时进行申诉或修改，逾期将无法领回划算金（建议平时经常登录网站查看站内信提醒哦！）<a style="color:#09f;display:block;" href="http://help.zhonghuasuan.com/buyer/category/63/81/124" target="_blank">如何获取订单编号？</a></p>'
						 +				'</td>'
						 + 			'</tr>'
						 + 		'</tbody>'
						 + '</table></form>';
		...
		pay_report : function(order_id,url) {
			art.dialog({
			    ...
			    ok: function(topWin){
					...
					var pay_appraised = $("input[name='pay_type']:checked").val(); 
					var val           = $('#val').val();
...
//			}, false);
		...
		refuse : function(oid) {
			...
			var html = [];
			html.push('<form class="woyao-show" method="post" enctype="multipart/form-data"><table>');
			html.push('<tr style="font-size:12px;"><th>标题:</th><td>', order.title, '</td></tr><br/><br/>');
		...
		/* 追加商品库存 [kza] */
		add_time : function(pid,url) {
			art.dialog.load(url + '&id=' + pid, {
			    title: '追加商品时间',
			    padding:0,
			    lock:true,
			    id:'add_time',
			    ...
			    ok: function(topWin){
			    	...
			    	$.post(site.site_root+'?m=member&c=merchantProduct&a=add_time',{
			    		id : pid,
			    		com_day : com_day,
			    	},function(ret){
			    		if (ret.status == 1){
