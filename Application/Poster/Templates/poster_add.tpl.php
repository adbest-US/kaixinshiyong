...
$show_dialog = $show_header = $show_validator = 1; 
...
<div class="subnav">
    ...
    <?php echo D('Node')->getsubmenu(MENUID); ?><span>|</span><a href="javascript:window.top.art.dialog({id:'setting',iframe:'?m=Poster&c=PosterSpace&a=setting', title:'<?php echo L('module_setting')?>', width:'540', height:'320'}, function(){var d = window.top.art.dialog({id:'setting'}).data.iframe;var form = d.document.getElementById('dosubmit');form.click();return false;}, function(){window.top.art.dialog({id:'setting'}).close()});void(0);"><em><?php echo L('module_setting')?></em></a>
    </div>
</div>
...
<tbody>
	...
	<tr>
		...
		<td><?php echo $form::date('poster[startdate]',date('Y-m-d H:i:s', NOW_TIME),1)?></td>
	</tr>
	<tr>
...
$(document).ready(function(){
	...
	$('#name').formValidator({onshow:"<?php echo L('please_input_name')?>",onfocus:"<?php echo L('name_three_length')?>",oncorrect:"<?php echo L('correct')?>"}).inputValidator({min:6,onerror:"<?php echo L('adsname_no_empty')?>"}).ajaxValidator({
		type:"get",
		url:"",
		data:"m=Poster&c=Poster&a=public_check_poster",
		datatype:"html",
		cached:false,
		async:'true',
