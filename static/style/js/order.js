...
var order = (function() {
	...
	return {
		...
		trial_url : function(oid) {
			...
			switch(_order.type) {
				case 'general': //普通下单
					buy_succes += '<p class="issue"><a href="'+site.site_root+'/help/?catid=79" target="_blank">普通下单常见问题</a></p><font class="font f_bg_yes font_2">恭喜您，已获得试用资格，请确认以下优惠哦！</font><ul class="hint_text_2"><p>请注意以下事项：</p><li>1、下单价：<em>'+_order.goods_price+'</em>，请在下单页面核对下单价是否一致。</li><li>2、试用完成后将返还试用金：<em>'+_order.goods_price+'</em> 元。</li><li>注意：获得资格后<em>'+ act_config.buyer_write_order_time+'小时内</em>不下单付款并返回填写订单号，本次订单将自动关闭。</li></ul></div>';
					break;
				case 'qrcode': //二维码下单
				...
				//	buy_succes += '<li>2、请使用<em>Ctrl+F 查找</em>旺旺名称:'+_order.contact_want+';进入店铺找到活动宝贝，然后进行下单。</li>';
					...
					buy_succes += '<li>2、搜索提示：<span style="color:red;">'+ _order.pro.goods_rule.keyword2 +'</li></ul>';
				...
				case 'ask' : //问答下单
					// buy_succes += '<p class="issue"><a href="'+site.site_root+'/help/?catid=81" target="_blank">答案下单常见问题</a></p><font class="font f_bg_yes font_2">抢购成功，请确认以下优惠哦！</font><ul class="hint_text_2"><p>请注意以下事项：</p><li>1、下单价：<em>'+_order.goods_price+'</em>，请在下单页面核对下单价是否一致。</li><li>2、返还划算金：<em>'+(_order.pro.goods_price-_order.pro.goods_price*_order.pro.goods_discount/10).toFixed(2)+'</em> 元，交易完成后，将返还给您金额。</li><li>注意：报名抢购后<em>'+ act_config.buyer_write_order_time+'分钟内</em>不下单付款并返回填写订单号，本次订单将自动关闭。</li></ul></div>';
					buy_succes += '<p class="issue"><a href="'+site.site_root+'/help/?catid=81" target="_blank">答案下单常见问题</a></p><font class="font f_bg_yes font_2">恭喜您，已获得试用资格，请确认以下优惠哦！</font><ul class="hint_text_2"><p>请注意以下事项：</p><li>1、下单价：<em>'+_order.goods_price+'</em>，请在下单页面核对下单价是否一致。</li><li>2、返还划算金：<em>'+_order.goods_price+'</em> 元，交易完成后，将返还给您金额。</li><li>注意：报名抢购后<em>'+ act_config.buyer_write_order_time+'分钟内</em>不下单付款并返回填写订单号，本次订单将自动关闭。</li></ul></div>';
					break;
			};
			if (_order.type == 'search') {
				okval = '去搜索下单';
